package com.example.dictionaryapp.features.feature_dictionary.data.repository
...
class WordInfoRepositoryImpl(
    ...
    override fun getWordInfoHistory(): Flow<Resource<List<WordInfo>>> = flow {
        emit(Resource.Loading())
        try {
            val wordInfoHistory = dao.getWordInfoHistory().map { it.toWordInfo() }
            emit(Resource.Success(wordInfoHistory))
        } catch (e: Exception) {
            emit(
                Resource.Error(
